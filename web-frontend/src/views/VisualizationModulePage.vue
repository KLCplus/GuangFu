...
import { Cloudy, Cpu, DataAnalysis, Document, Grid, Sunny } from '@element-plus/icons-vue'
...
const modules: Record<string, ModuleDefinition> = {
  model: {
    ...
    description: '选择预测模型与输入数据，执行光伏功率推演并查看结果曲线。', service: '模型推理服务', icon: Cpu
  },
  cloud: {
    component: 'cloud', code: 'CLD', overline: 'CLOUD IMAGE FORECAST', title: '云图预测控制台',
    description: '上传连续云图帧，追踪云团演变并生成后续时序预测。', service: '云图推演服务', icon: Cloudy
  },
  weather: {
    component: 'weather', code: 'WX', overline: 'WEATHER OBSERVATION', title: '气象监测中心',
    description: '汇总电站实时天气、关键气象指标与短期预报。', service: '气象数据链路', icon: Cloudy
  },
  marketplace: {
    component: 'marketplace', code: 'MKT', overline: 'MODEL MARKETPLACE', title: '模型能力广场',
    description: '按任务、输入模态与模型架构筛选可用预测能力。', service: '模型目录服务', icon: Grid
  },
  'api-overview': {
    component: 'api', code: 'API', overline: 'OPEN API SERVICE', title: 'API 开放平台',
    description: '查看开放能力、接口资源与模型服务接入方式。', service: '开放接口网关', icon: Document
  },
  'api-keys': {
    component: 'api', code: 'KEY', overline: 'ACCESS KEY CONTROL', title: 'API Key 控制台',
    description: '创建、启停和管理 API 访问凭证及安全状态。', service: '凭证管理服务', icon: Document
  },
  'api-billing': {
    component: 'api', code: 'WAL', overline: 'ACCOUNT BILLING CENTER', title: '余额与流水中心',
    description: '查看账户余额、资金变动与开放服务消费记录。', service: '计费账户服务', icon: Document
  },
  'api-usage': {
    component: 'api', code: 'USE', overline: 'API USAGE TELEMETRY', title: 'API 使用统计',
    description: '分析调用趋势、模型分布、Key 使用量与调用日志。', service: '调用遥测服务', icon: DataAnalysis
  },
  reports: {
    component: 'reports', code: 'AGT', overline: 'ENERGY ANALYSIS AGENT', title: 'Agent 分析工作台',
    description: '通过智能会话编排数据工具、分析过程与报告输出。', service: '智能分析服务', icon: DataAnalysis
  },
  pvoutput: {
    component: 'pvoutput', code: 'OP', overline: 'PUBLIC PV OBSERVATORY', title: '公开电站观测台',
    description: '浏览公开光伏系统的实时功率、天气与发电趋势。', service: 'PVOutput 数据链路', icon: Sunny
  }
}
...
<template>
  <section class="visual-module-page" :class="`module-${moduleDefinition.component}`">
    <header class="module-command-heading">
      ...
      <div class="module-telemetry" aria-label="模块运行状态">
        <article><small>SERVICE CHANNEL</small><strong>{{ moduleDefinition.service }}</strong><em><i></i>在线</em></article>
        <article><small>DATA MODE</small><strong>实时接口</strong><em><i></i>同步</em></article>
        <article><small>OPERATION STATE</small><strong>功能可用</strong><em><i></i>正常</em></article>
      </div>
