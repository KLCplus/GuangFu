...
import { confirmEmailChange, deleteAvatar, deleteFace, enrollFace, getFaceStatus, sendEmailChangeCode, uploadAvatar } from '../api/user'
...
interface ProfileForm { nickname: string; phoneCountryCode: string; phone: string; gender: number }
interface PasswordForm { oldPassword: string; newPassword: string; confirmPassword: string }
interface EmailChangeForm { newEmail: string; code: string; currentPassword: string }
...
const emailSending = ref(false), emailConfirming = ref(false), emailCodeSent = ref(false), emailCountdown = ref(0)
const actionLoadingId = ref<number | null>(null)
const overview = ref<ProfileOverview | null>(null)
const loadError = ref('')
const editing = ref(false), passwordDialogVisible = ref(false), emailDialogVisible = ref(false)
const faceFile = ref<File>(), facePreview = ref(''), videoRef = ref<HTMLVideoElement>(), cameraActive = ref(false)
let cameraStream: MediaStream | undefined
const profileForm = reactive<ProfileForm>({ nickname: '', phoneCountryCode: '+86', phone: '', gender: 0 })
const passwordForm = reactive<PasswordForm>({ oldPassword: '', newPassword: '', confirmPassword: '' })
const emailForm = reactive<EmailChangeForm>({ newEmail: '', code: '', currentPassword: '' })
const callingCodes = [
...
let emailCountdownTimer: number | undefined
...
onBeforeUnmount(() => { window.removeEventListener('pv:user-profile-updated', fetchOverview); stopFaceCamera(); clearFaceSelection(); stopEmailCountdown() })

function selectSection(section: Section) { editing.value = false; void router.push({ path: props.basePath, query: section === 'basic' ? {} : { section } }) }
...
function fillProfileForm(value: UserProfile) {
  profileForm.nickname=value.nickname ?? ''; profileForm.gender=value.gender ?? 0
  const matched=callingCodes.map(item=>item.value).sort((a,b)=>b.length-a.length).find(code=>value.phone?.startsWith(code))
  profileForm.phoneCountryCode=matched ?? '+86'; profileForm.phone=matched ? (value.phone ?? '').slice(matched.length) : (value.phone ?? '')
}
...
async function saveProfile() {
  ...
  const phone=profileForm.phone.replace(/[\s()-]/g,'')
  if (phone && profileForm.phoneCountryCode==='+86' && !/^1[3-9]\d{9}$/.test(phone)) return ElMessage.warning('请输入有效的中国大陆 11 位手机号')
  if (phone && profileForm.phoneCountryCode!=='+86' && !/^\d+$/.test(phone)) return ElMessage.warning('国际手机号只能包含数字')
  if (phone && profileForm.phoneCountryCode!=='+86' && `${profileForm.phoneCountryCode}${phone}`.replace('+','').length>15) return ElMessage.warning('国际手机号不能超过 15 位')
  if (phone && profileForm.phoneCountryCode!=='+86' && `${profileForm.phoneCountryCode}${phone}`.replace('+','').length<8) return ElMessage.warning('请输入有效的国际手机号')
  saving.value=true
  try {
    await saveUserProfile({ nickname: profileForm.nickname.trim(), phone: phone ? `${profileForm.phoneCountryCode}${phone}` : '', gender: profileForm.gender } as UpdateProfilePayload)
    await fetchOverview()
    const saved=profile.value
    if(saved) userStore.setUserInfo({ userId:saved.userId, username:saved.username, nickname:saved.nickname, email:saved.email, roles:saved.roles, status:saved.status })
...
function resetEmailForm() { emailForm.newEmail=''; emailForm.code=''; emailForm.currentPassword=''; emailCodeSent.value=false; stopEmailCountdown() }
function stopEmailCountdown() { if(emailCountdownTimer!==undefined) window.clearInterval(emailCountdownTimer); emailCountdownTimer=undefined; emailCountdown.value=0 }
function startEmailCountdown() { stopEmailCountdown(); emailCountdown.value=60; emailCountdownTimer=window.setInterval(()=>{ emailCountdown.value-=1; if(emailCountdown.value<=0) stopEmailCountdown() },1000) }
function validEmail(value:string) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) }
async function requestEmailCode() {
  const newEmail=emailForm.newEmail.trim().toLowerCase()
  if(!validEmail(newEmail)) return ElMessage.warning('请输入有效的新邮箱')
  if(!emailForm.currentPassword) return ElMessage.warning('请输入当前密码')
  emailSending.value=true
  try { await sendEmailChangeCode({newEmail,currentPassword:emailForm.currentPassword}); emailCodeSent.value=true; startEmailCountdown(); ElMessage.success('验证码已发送，验证前原邮箱保持不变') }
  catch(error) { emailCodeSent.value=false; ElMessage.error(error instanceof Error?error.message:'验证码发送失败') }
  finally { emailSending.value=false }
}
async function submitEmailChange() {
  const newEmail=emailForm.newEmail.trim().toLowerCase()
  if(!validEmail(newEmail)||!emailForm.code.trim()||!emailForm.currentPassword) return ElMessage.warning('请完整填写新邮箱、验证码和当前密码')
  emailConfirming.value=true
  try { await confirmEmailChange({newEmail,code:emailForm.code.trim(),currentPassword:emailForm.currentPassword}); await fetchOverview(); emailDialogVisible.value=false; resetEmailForm(); ElMessage.success('邮箱已验证并更新') }
  catch(error) { await fetchOverview(); ElMessage.error(error instanceof Error?error.message:'邮箱修改失败') }
  finally { emailConfirming.value=false }
}
...
function maskPhone(value?:string) { return value && value.length>7 ? `${value.slice(0,Math.min(5,value.length-4))}****${value.slice(-4)}` : value || '未填写' }
function emailStatus(value?:UserProfile) { return value?.email ? (value.emailVerified?'邮箱已验证':'邮箱未验证') : '邮箱未绑定' }
    ...
    <template v-if="profile">
      ...
      <div class="settings-layout"><nav class="settings-nav"><button v-for="item in sections" :key="item.key" :class="{active:activeSection===item.key}" @click="selectSection(item.key)"><el-icon><component :is="item.icon" /></el-icon>{{ item.label }}</button></nav>
        <main class="settings-content">
          <section v-if="activeSection==='basic'" class="settings-panel"><header><div><h2>个人资料</h2><p>管理头像、昵称、性别和未验证的联系手机号。</p></div><el-button v-if="!editing" @click="editing=true">编辑资料</el-button></header><template v-if="!editing"><dl class="detail-list"><dt>头像</dt><dd><el-avatar :size="44" :src="profile.avatarUrl">{{ (profile.nickname || profile.username).slice(0,1) }}</el-avatar><el-upload :show-file-list="false" accept="image/jpeg,image/png,image/webp" :http-request="changeAvatar"><el-button size="small" :loading="avatarSaving">更换头像</el-button></el-upload><el-button v-if="profile.avatarUrl" size="small" text type="danger" :loading="avatarSaving" @click="removeAvatar">删除</el-button><small>JPG、PNG、WebP，最大 2MB</small></dd><dt>用户名</dt><dd>{{ profile.username }}</dd><dt>昵称</dt><dd>{{ profile.nickname || '未设置' }}</dd><dt>邮箱</dt><dd>{{ profile.email || '未绑定' }} <el-tag size="small" :type="profile.emailVerified?'success':'info'">{{ profile.email ? (profile.emailVerified?'已验证':'未验证') : '未绑定' }}</el-tag><small>请在“安全与登录”中修改</small></dd><dt>联系手机号</dt><dd>{{ maskPhone(profile.phone) }} <el-tag v-if="profile.phone" size="small" type="warning">已填写，未验证</el-tag></dd><dt>性别</dt><dd>{{ genderLabel(profile.gender) }}</dd><dt>角色</dt><dd>{{ roleText(profile.roles) }}</dd><dt>注册时间</dt><dd>{{ profile.createdAt }}</dd></dl></template><el-form v-else class="profile-form" label-position="top"><el-form-item label="昵称" required><el-input v-model="profileForm.nickname" maxlength="32" show-word-limit /></el-form-item><el-form-item label="联系手机号"><div class="phone-input"><el-select v-model="profileForm.phoneCountryCode"><el-option v-for="item in callingCodes" :key="item.value" :label="item.label" :value="item.value" /></el-select><el-input v-model="profileForm.phone" maxlength="15" inputmode="tel" placeholder="请输入号码" /></div><small class="field-help">仅作为联系方式，以国际格式保存；当前未接入短信验证。</small></el-form-item><el-form-item label="性别"><el-radio-group v-model="profileForm.gender"><el-radio :value="0">未设置</el-radio><el-radio :value="1">男</el-radio><el-radio :value="2">女</el-radio></el-radio-group></el-form-item><footer><el-button @click="cancelEdit">取消</el-button><el-button type="primary" :loading="saving" @click="saveProfile">保存资料</el-button></footer></el-form></section>
          <section v-else-if="activeSection==='security'" class="settings-panel"><header><div><h2>安全与登录</h2><p>管理密码、已验证邮箱和人脸认证。</p></div></header><div class="setting-row"><div><h3>密码</h3><p>用于保护账户登录安全。</p></div><el-button @click="passwordDialogVisible=true">修改密码</el-button></div><div class="setting-row"><div><h3>登录邮箱</h3><p>{{ profile.email || '未绑定' }} · {{ profile.email ? (profile.emailVerified?'已验证':'未验证') : '未绑定' }}</p></div><div class="row-actions"><el-tag :type="profile.emailVerified?'success':'warning'">{{ emailStatus(profile) }}</el-tag><el-button @click="emailDialogVisible=true">{{ profile.email?'更换邮箱':'绑定邮箱' }}</el-button></div></div><div class="setting-row"><div><h3>联系手机号</h3><p>{{ maskPhone(profile.phone) }} · 不用于找回密码、验证码登录或身份认证</p></div><el-tag :type="profile.phone?'warning':'info'">{{ profile.phone?'已填写，未验证':'未填写' }}</el-tag></div><div class="face-setting"><div class="setting-row"><div><h3>人脸认证</h3><p>状态：{{ faceStatus?.enrolled?'已录入':'未录入' }}<template v-if="faceStatus?.enrolledAt"> · 录入时间：{{ faceStatus.enrolledAt }}</template></p></div><el-tag :type="faceStatus?.enrolled?'success':'info'">{{ faceStatus?.enrolled?'可用':'未启用' }}</el-tag></div><div v-if="cameraActive" class="camera"><video ref="videoRef" autoplay muted playsinline /><div><el-button type="primary" @click="captureFacePhoto">拍照</el-button><el-button @click="stopFaceCamera">关闭摄像头</el-button></div></div><div v-if="facePreview" class="preview"><img :src="facePreview" alt="人脸预览" /><el-button text type="danger" @click="clearFaceSelection">移除</el-button></div><div class="face-actions"><label class="file-button"><input type="file" accept="image/*" @change="selectFaceFile" />选择图片</label><el-button @click="openFaceCamera">打开摄像头</el-button><el-button type="primary" :loading="faceSaving" :disabled="!faceFile" @click="enrollSelectedFace">{{ faceStatus?.enrolled?'更新人脸':'录入人脸' }}</el-button><el-button text type="danger" :loading="faceRevoking" :disabled="!faceStatus?.enrolled" @click="revokeFaceAuth">撤销</el-button></div></div></section>
