...
## 2026-07-14 用户资料、邮箱换绑与联系手机号安全修复

### 原问题与边界

- 原 `PUT /api/user/profile` 的 `UpdateProfileRequest` 同时接收昵称、邮箱、手机号、头像和性别，`UserService.updateProfile` 对邮箱只做唯一性检查便直接 `setEmail`，没有验证码步骤和独立验证状态，因此 PC、小程序和 Agent 资料工具都可以绕过验证替换邮箱。
- 原手机号只检查是否重复，没有格式或国家/地区区号校验；PC 和小程序又把“字段非空”展示为“已绑定”，实际没有短信验证能力。
- `sys_user` 原来没有邮箱验证状态字段。此次只增加 `email_verified`，没有重构用户表或新增验证码表；手机号暂不增加虚假的验证字段，接口明确返回 `phoneVerified=false`。

### 后端修复

- 通用资料 DTO 和 Agent 的资料修改工具移除邮箱写参数，只保留昵称、联系手机号、头像和性别。即使旧客户端仍向资料接口提交 `email`，该字段也不会进入更新逻辑；专项接口测试确认昵称可更新而原邮箱保持不变。
- 新增 `POST /api/user/security/email/code` 与 `PUT /api/user/security/email`。发送和确认两步都要求当前密码；发送前及确认写库前均重新检查新邮箱格式、与当前邮箱不同、全局唯一。
- 邮箱换绑复用现有 `VerificationCodeService`、`DistributedStateService` 和 `EmailService`，继续使用安全随机码、SHA-256 哈希缓存、配置化有效期、重发冷却、每日发送上限和错误尝试次数；换绑使用 `email-change:{userId}:{newEmail}` 独立用途键，避免与注册、登录或找回密码验证码混用。
- 只有验证码校验成功后才在同一事务中更新 `email` 和 `email_verified=1`。验证码发送阶段不修改用户表；SMTP 未配置或投递失败现在返回明确错误，不再由接口假装“已发送”。邮件账号、授权码和密钥仍只从现有环境配置读取。
- 普通密码注册时填写的邮箱保存为未验证；邮箱验证码注册保存为已验证；邮箱验证码登录在验证码成功后补记已验证。密码找回和重置只接受 `email_verified=1` 的邮箱，未验证邮箱不能仅凭字段非空成为安全凭据。
- 新增幂等 `UserSecuritySchemaMigration`：旧库仅在缺少字段时执行 `ALTER TABLE sys_user ADD email_verified ... DEFAULT 0`，不改写原邮箱；全量 `init.sql` 和 H2 测试结构同步补齐字段。
- 联系手机号统一规范为 E.164。后端当前支持 `+86`、`+1`、`+44`、`+81`、`+82`；`+86` 必须为 `1[3-9]` 开头的 11 位大陆手机号，其他已支持区号要求完整号码总长 8–15 位且只含数字。兼容输入未带区号的合法大陆 11 位号码，并规范保存为 `+86...`。

### PC 端

- “个人资料”不再提供邮箱输入框，邮箱只读展示并引导到“安全与登录”；联系手机号增加国家/地区区号选择和与后端一致的前置校验。
- “安全与登录”增加独立邮箱换绑对话框：输入新邮箱和当前密码、发送验证码、输入验证码、点击“验证并更新邮箱”。验证码发送后只提示“原邮箱保持不变”，不会提前写入页面资料或显示换绑成功。
- 邮箱标签严格读取 `emailVerified`；手机号显示“未填写”或“已填写，未验证”，并明确不用于找回密码、验证码登录和身份认证。
- 资料保存与邮箱确认成功后均重新请求真实账户概览；邮箱确认失败也重新读取资料，前端不保留待换邮箱或假成功状态。

### 微信小程序

- 在后端和 PC 链路验证后，同步移除个人资料页的直接邮箱编辑，账户安全页接入相同的“当前密码 + 新邮箱验证码”换绑流程。
- 联系手机号页同步增加上述五个区号选项和本地格式校验，保存后重新拉取资料；“我的”、个人资料和安全页都改为读取 `emailVerified`，不再根据邮箱非空显示“已绑定”。
- 小程序仍未接入短信发送、短信验证码登录、手机号密码找回或手机号身份认证；手机号始终只是未验证联系方式。

### 验证结果与待人工验收

- 后端 Maven 编译通过；`UserServiceTest` 与 `AuthServiceTest` 专项回归通过，覆盖通用接口不能改邮箱、验证码确认前原邮箱不变、确认后邮箱和验证状态同时更新、大陆号码规范化及非法号码拒绝。
- PC `vue-tsc --noEmit` 通过。Vite 在受限沙箱内因 Windows 子进程 `spawn EPERM` 失败，沙箱外整包构建又在 120 秒无输出后超时，因此不将本轮写成生产打包已通过。
- 小程序全部 JavaScript 通过 `node --check`，全部 JSON 可解析；仍需在微信开发者工具中验证换绑表单、60 秒倒计时、长邮箱、五种区号、小屏布局和真实 SMTP 收件。
- 现有邮箱验证码注册产生的无用户可知密码账户，以及仅 OAuth 登录且没有本地密码的账户，当前无法满足“当前密码”校验；本轮没有扩展为无密码账户的二次认证方案。普通密码账户的邮箱换绑链路已完整闭环。

## 2026-07-15 模型上线、资讯清洗与小程序视觉统一

### 模型状态与 API 概览

- 实际核对本地 MySQL `model_info` 后，以稳定 `model_code` 确认本次可调用模型为 `DLinear`、`PatchTST`、`iTransformer`、`TimeXer`、`TimeMixer`、`TSMixer`、`Transformer`、`CNN_MLP`（页面名称 CNN+MLP）。
- 新增幂等增量脚本 `backend/src/main/resources/sql/update_online_models_20260715.sql`，只更新 `status` 和 `updated_at`：上述 8 个设为 `ONLINE`，其余未删除模型设为 `OFFLINE`，不修改 `marketplace_visible` 或模型广场元数据。脚本已同步执行到本地库，实际更新 1 行（`CNN_MLP`），接口重启后实测返回 8 个在线模型。
- API 概览新增统一 `onlineModels` 数据源，数量卡片、紧凑模型列表、搜索和空状态均只读取 `ONLINE`；列表最多显示 6 个并保留“查看全部模型”。头像直接复用 PC 模型广场 `getModelIcon(modelCode, modelType)`，未匹配时沿用公共类型/统一兜底头像。

### 资讯与消息、摘要和正文

- visualization-ui 左侧导航、路由标题、模块主标题和详情标题统一为“资讯与消息 / 资讯详情”，说明改为“汇总行业资讯、政策动态与平台消息”，路由地址保持 `/visualization-ui/news` 不变。
- 后端新增 `NewsContentNormalizer`，作为 PC 和小程序共同的数据规则：空摘要不返回；摘要与正文开头包含或相似度达到阈值时不返回；独立摘要继续返回。内部运维指南抽查仍保留有效摘要。
- 外部正文在抓取入库前移除导航、分享、推荐、表单等节点，并清除含“目录项的基本信息、公开事项名称、索引号、主办单位、制发日期”等标签的元数据表、重复标题、空表和包裹正文的单单元格布局表。`NewsService.toVO` 对历史库再次执行同一清理，因此无需逐条写死或立即改写历史数据。
- PC `newsContent.ts` 与小程序 `news-content.js` 只作为通用展示兜底，负责重复/元数据型摘要判断；小程序额外兼容旧后端内容中的元数据表和首个重复标题。主要清理仍以后端为准。
- 本地历史数据共有 28 条外部资讯，其中 4 条国家能源局正文命中旧元数据结构。重启本地后端后实测新闻 `10`：摘要为空、元数据不存在、重复标题不存在、正文不再以布局表开头；内部新闻 `21` 的独立运维摘要继续显示。无需人工逐条清理，若要求数据库原始 HTML 也永久改写，可在备份后另做离线回填，本轮接口兼容和后续入库已闭环。

### 小程序模型与视觉

- 小程序公开模型 VO 补充 `modelCode`；新增 `utils/model-icons.js`，将 PC 现有 19 个专属模型 SVG 原样栅格化为 128×128 本地 PNG，并提供统一 fallback。模型列表和详情均按 `modelCode` 取图，不再生成首字母头像。
- 四个主页面移除 `OPEN PLATFORM`、`MODEL CATALOG`、`INFORMATION & MESSAGE` 等英文 eyebrow 和无业务含义装饰，保留“开放平台概览、模型广场、资讯与消息、我的”等中文标题及简短说明。
- `app.json` 为概览、模型、资讯、我的配置统一线性语义图标及普通/选中 PNG；普通态为灰蓝，选中态为品牌蓝。
- 视觉调整遵循“小程序快速浏览”而非 PC 缩小版：全局浅灰白画布、白卡片、深蓝黑标题和灰蓝说明；概览与账户页移除大面积高饱和蓝，钱包、身份卡和模型详情改为浅色卡片，品牌蓝仅用于按钮、选中状态和数据强调，边框、阴影与圆角同步收敛。

### 验证

- 后端 `NewsContentNormalizerTest` 通过，覆盖政府元数据、重复标题、单单元格布局表、重复摘要和独立摘要；本地 Spring Boot 已重启并通过真实接口验证 8 个在线模型与历史资讯兼容清理。
- PC `vue-tsc` 与 Vite 生产构建通过；只有既有依赖纯注释和大 chunk 提示，没有构建错误。
- 小程序全部 JavaScript 通过 `node --check`，`app.json` 可解析，8 个 Tab 状态图标路径和 20 个模型头像资源（19 个专属头像加 1 个 fallback）全部存在；仍需在微信开发者工具和真机检查 Tab 图标实际像素、小屏换行及富文本表格滚动。
- 本轮没有修改 `.env`、`backend/.env.local` 或启动脚本；这两个环境文件进入任务前已有本地修改，继续保留且不纳入本次提交范围。

### 2026-07-15 站内消息数据补充

- 新增幂等脚本 `backend/src/main/resources/sql/seed_user_notifications_20260715.sql`，为当前启用用户补充 `NOTICE`、`MODEL_UPDATE`、`ALERT`、`SYSTEM` 四类未读消息。
- 脚本按 `user_id + notification_type + title` 检查已有记录，不覆盖已有消息或已读状态；异常类内容只说明提醒能力已启用，不伪造真实业务故障。
- 本地数据库用户 `test2` 已写入 4 条，执行后消息总数与未读数均为 4。

## 2026-07-15 小程序“我的”模块与测试充值统一

### 二级页面与账户入口

- 个人资料、安全与登录、钱包与消费、API 权益四页统一为浅灰画布、白色卡片、深色标题、灰蓝辅助文字和克制边框；移除旧版大面积蓝色渐变，品牌蓝仅用于主要按钮、状态和重点数据。此次遵循现有小程序卡片与间距体系，没有把 PC 页面机械缩放到移动端。
- “我的”首页原“资、安、钱、权”文字圆标替换为四个 64×64 本地 PNG 线性图标：资料卡、盾牌锁、钱包、API 节点。图标统一使用透明底品牌蓝线条，由页面提供柔和浅蓝容器，不依赖远程资源。
- 个人资料页头像区改为白色轻量卡片，继续支持当前头像查看和真实上传更换；移除右侧红色“删除当前头像”入口，后端原删除接口未被破坏，但不再作为页面突出操作。
- API 权益页继续并行读取真实 API Key 和权益接口，保留脱敏 Key、启停状态、日额度、模型、已用/总额度和有效期；失败和空状态按各自接口结果展示，没有补造额度或到期时间。

### 安全与登录交互

- 页面默认只显示登录邮箱及验证状态、联系手机号及验证状态、登录密码保护说明和对应操作入口。邮箱换绑与密码修改采用页面内单区按需展开，同一时间只展开用户主动选择的操作，降低默认信息密度。
- 邮箱换绑仍调用原 `发送验证码 + 当前密码` 和 `验证码确认 + 当前密码` 两步真实接口；验证码通过前不改变邮箱。密码修改继续校验原密码、新密码长度和二次确认，成功后清除登录态并要求重新登录。
- 当前后端没有短信验证能力，因此手机号状态如实显示“未验证”，入口跳转个人资料完善联系方式，不展示无法兑现的短信验证表单。

### 开放平台概览标题

- 概览页保留现有轻量白卡布局、近 30 天筛选和统计数据；将“开放平台概览”降为栏目名称，以“每一次调用，都看得见”为主要标题，并保留“掌握调用状态、账户资源与近期记录”的说明。没有恢复旧版高饱和蓝色 Hero。

### PC 与小程序测试充值

- 核对确认 PC 原充值会真实调用 `POST /api/open/wallet/recharge`：后端事务创建 `open_recharge_order`、增加 `open_wallet_account.balance`，并写入 `open_wallet_record`；但没有连接微信、支付宝或银行卡支付，因此属于测试入账，不是真实支付。
- 后端将当前可接受渠道收口为 `MOCK`，新流水标题为“测试充值”，备注明确“不产生真实扣款”；数据库订单、余额和流水逻辑保持真实，不在前端模拟余额。
- PC 的入口、弹窗、成功/失败提示和说明统一改为“测试充值”，移除支付宝、微信、银行等容易造成误解的模拟选项，只提交 `MOCK`。
- 小程序钱包页接入同一真实后端接口，提供 50、100、200、500 元常用金额与 1–100000 元自定义金额；确认弹窗再次提示不会真实扣款，成功后重新请求钱包，刷新余额和资金流水。概览页“测试充值”入口跳转钱包页。
- 仍需后端/支付系统补充真实支付下单、支付渠道签名、支付结果回调、订单状态机、幂等与退款后，才可将能力表述为正式充值；本轮没有使用 mock 数据掩盖该缺口。

### 验证边界

- 小程序相关页面及请求封装均通过 `node --check`，四个本地图标均为可读取的 64×64 PNG；小程序项目没有 npm 构建脚本，仍需微信开发者工具检查页面内展开、输入法、自定义金额、小屏排版及真机像素表现。
- PC `vue-tsc` 与 Vite 生产构建通过；仅有既有依赖纯注释和大 chunk 提示，没有构建错误。
- 后端使用项目内 Maven 仓库完成全量测试和编译：94 项运行、0 失败、0 错误、2 跳过，`BUILD SUCCESS`。第一次直接使用机器级 Maven 仓库时因 `D:\mymavenrepo` 目录不可写失败，改用项目内仓库后通过，未修改 Maven 或环境配置。
