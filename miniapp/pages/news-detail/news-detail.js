...
const { visibleSummary, cleanLegacyContent } = require('../../utils/news-content')
Page({
  ...
  async loadDetail() {
    ...
    try {
      ...
      const cleanedContent = cleanLegacyContent(item.content, item.title)
      const richContent = this.richContent(cleanedContent)
      const summary = visibleSummary(item.summary, cleanedContent)
      this.setData({ news: { ...item, summary, richContent, typeLabel, sourceText: item.sourceName || '光伏智云平台', sourceUrl: this.safeUrl(item.sourceUrl), attachmentUrl: this.safeUrl(item.attachmentUrl), timeText: dateTime(item.sourcePublishedAt || item.publishedAt || item.createdAt), effectiveText: item.effectiveAt ? dateTime(item.effectiveAt) : '', expiresText: item.expiresAt ? dateTime(item.expiresAt) : '' }, loading: false })
