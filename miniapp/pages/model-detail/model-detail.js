...
Page({
  ...
  async loadDetail() {
    ...
    try {
      ...
      const item = await (loggedIn ? modelApi : miniappModelApi).detail(this.modelId)
      const lists = [
  ...
  showActionStatus() {
    if (!this.data.model.loggedIn) {
      wx.navigateTo({ url: '/pages/login/login' })
      return
    }
    wx.showModal({ title: '功能说明', content: '模型购买暂未开放。页面不会模拟购买或试用成功；后续以真实业务接口为准。', showCancel: false })
  }
})
