...
class PatchMerging(nn.Module):
    """Patch Merging Layer compatible with checkpoints saved by older timm.

    Downsamples features by merging 2x2 neighboring patches. Accepts
    ``input_resolution`` and ``dim`` (the legacy timm signature) and stores
    ``norm``/``reduction`` under an ``op`` submodule so that the state-dict
    keys (``op.norm.*`` / ``op.reduction.*``) line up with the saved weights.
    Input/Output are in the ``(B, L, C)`` sequence format used by SwinLSTM.
    """
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        self.op = nn.Module()
        self.op.reduction = nn.Linear(4 * dim, 2 * dim, bias=False)
        self.op.norm = norm_layer(4 * dim)
...
class STB(SwinTransformerBlock):
    def __init__(self, index, dim, input_resolution, depth, num_heads, window_size, 
        ...
        # Normalize tuple attrs (new timm stores them as 2-tuples) for the
        # custom forward pass below, which expects plain ints.
        if isinstance(self.shift_size, (tuple, list)):
            self.shift_size = self.shift_size[0]
        # New timm uses drop_path1/drop_path2; expose a single drop_path alias.
    ...
    def forward(self, x, hx=None):
        ...
        ws = self.window_size[0] if isinstance(self.window_size, (tuple, list)) else self.window_size
        x_windows = x_windows.view(-1, ws * ws, C)  # num_win*B, window_size*window_size, C
