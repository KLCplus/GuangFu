"""iTransformer 模型训练+测试+SHAP 脚本

从 Time-Series-Library 改造迁移，使用 TSL 的 iTransformer 模型架构。
...
CKPT_DIR = Path("/root/code_compare/TSL/iTransformer/checkpoints")
RESULT_DIR = Path("/root/code_compare/TSL/iTransformer/result")
...
class PowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [30] → [30, 1] for iTransformer input format
...
def model_forward(model, x_enc, device):
    """iTransformer 前向：x_enc [B, 30, 1] → [B, 6]"""
...
def test(model, device):
    ...
    try:
        ...
        # iTransformer: inverted embedding + encoder + projection
        # inverted embedding: seq_len * d_model (per variable)
        embed_macs = ENC_IN * SEQ_LEN * D_MODEL
        # encoder: e_layers * (self_attn + FFN)
        # self_attn on N=enc_in tokens: 4 * enc_in * d_model * d_model
        attn_macs = 4 * ENC_IN * D_MODEL * D_MODEL
        ffn_macs = 2 * ENC_IN * D_MODEL * D_FF
        encoder_macs = E_LAYERS * (attn_macs + ffn_macs)
        # projection: d_model * pred_len (per variable)
        proj_macs = ENC_IN * D_MODEL * PRED_LEN
        macs = embed_macs + encoder_macs + proj_macs
...
def write_result_md(train_m, test_m):
    lines = []
    lines.append("# iTransformer 训练与测试结果\n")
    lines.append(
