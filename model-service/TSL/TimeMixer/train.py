"""TimeMixer 模型训练+测试+SHAP 脚本

从 Time-Series-Library 改造迁移，使用 TSL 的 TimeMixer 模型架构。
...
CKPT_DIR = Path("/root/code_compare/TSL/TimeMixer/checkpoints")
RESULT_DIR = Path("/root/code_compare/TSL/TimeMixer/result")
...
class PowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [30] → [30, 1] for TimeMixer input format
...
def model_forward(model, x_enc, device):
    """TimeMixer 前向：x_enc [B, 30, 1] → [B, 6]

    TimeMixer 的 forecast() 内部处理多尺度下采样、分解、混合，
    不需要 x_dec/x_mark_dec，直接传 None 即可。
    输出 [B, pred_len, enc_in] → squeeze(-1) → [B, 6]
    """
    out = model(x_enc, None, None, None)  # [B, pred_len, enc_in]
    ...
    try:
        ...
        class MacWrapper(nn.Module):
            ...
            def forward(self, x_enc):
                return self.model(x_enc, None, None, None).squeeze(-1)
        ...
        # TimeMixer 手动估算 MACs
        # 多尺度: seq_len // down_sampling_window^i for i in 0..down_sampling_layers
        scales = []
        for i in range(DOWN_SAMPLING_LAYERS + 1):
            scales.append(SEQ_LEN // (DOWN_SAMPLING_WINDOW ** i))
        # scales = [30, 15, 7, 3]

        macs = 0
        for scale_len in scales:
            # embedding: 1 -> d_model (per time step, per variable=1)
            macs += scale_len * 1 * D_MODEL
            # e_layers PDM blocks:
            for _ in range(E_LAYERS):
                # out_cross_layer: d_model -> d_ff -> d_model (per time step)
                macs += scale_len * D_MODEL * D_FF
                macs += scale_len * D_FF * D_MODEL
            # predict_layer: scale_len -> pred_len (per d_model)
            macs += D_MODEL * scale_len * PRED_LEN
            # projection_layer: d_model -> 1 (per pred_len)
            macs += PRED_LEN * D_MODEL * 1

        # MultiScaleSeasonMixing & MultiScaleTrendMixing (down_sampling_layers layers)
        for i in range(DOWN_SAMPLING_LAYERS):
            len_in = scales[i]
            len_out = scales[i + 1]
            # Season: 2 Linear (len_in -> len_out, len_out -> len_out) per variable
            macs += 1 * len_in * len_out + 1 * len_out * len_out
            # Trend: 2 Linear (len_out -> len_in, len_in -> len_in) per variable
            macs += 1 * len_out * len_in + 1 * len_in * len_in
...
def run_shap(model, device):
    ...
    class PowerWrapper(nn.Module):
        ...
        def forward(self, x):
            # x: [B, 30, 1]
            out = self.model(x, None, None, None)  # [B, 6, 1]
            return out.squeeze(-1)  # [B, 6]
...
def write_result_md(train_m, test_m):
    lines = []
    lines.append("# TimeMixer 训练与测试结果\n")
    lines.append(
    ...
    lines.append(f"| train_down_sampling_layers | {train_m['down_sampling_layers']} |")
    lines.append(f"| train_down_sampling_window | {train_m['down_sampling_window']} |")
    lines.append(f"| train_down_sampling_method | {train_m['down_sampling_method']} |")
    lines.append(f"| train_channel_independence | {train_m['channel_independence']} |")
    lines.append(f"| train_decomp_method | {train_m['decomp_method']} |")
    lines.append(f"| train_moving_avg | {train_m['moving_avg']} |")
    lines.append(f"| train_top_k | {train_m['top_k']} |")
