"""PatchTST 模型训练+测试+SHAP 脚本

从 Time-Series-Library 改造迁移。
...
CKPT_DIR = Path("/root/code_compare/TSL/patchtst/checkpoints")
RESULT_DIR = Path("/root/code_compare/TSL/patchtst/result")
...
class PowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [30] → [30, 1] for PatchTST input format
...
def build_model():
    ...
    model = PatchTSTModel(configs, patch_len=PATCH_LEN, stride=STRIDE)
    return model
...
def model_forward(model, x_enc, device):
    """PatchTST 前向：x_enc [B, 30, 1] → [B, 6]"""
    B = x_enc.shape[0]
    x_mark_enc = None
    x_dec = torch.zeros(B, PRED_LEN, ENC_IN, device=device)
    x_mark_dec = None
    out = model(x_enc, x_mark_enc, x_dec, x_mark_dec)  # [B, 6, 1]
...
def test(model, device):
    ...
    try:
        ...
        dummy_dec = torch.zeros(1, PRED_LEN, ENC_IN).to(device)
        class MacWrapper(nn.Module):
            def __init__(self, model):
                super().__init__()
                self.model = model
            def forward(self, x_enc, x_dec):
                return self.model(x_enc, None, x_dec, None).squeeze(-1)
        macs, _ = profile(MacWrapper(model).to(device), inputs=(dummy_x, dummy_dec), verbose=False)
        if macs == 0:
            raise ValueError("thop returned 0")
    except Exception as e:
        print(f"  thop 不可用({e})，手动估算 MACs")
        # PatchTST MACs 估算：patch_embedding + encoder + head
        patch_num = int((SEQ_LEN - PATCH_LEN) / STRIDE + 2)
        # patch projection: patch_len * d_model
        patch_proj_macs = patch_num * PATCH_LEN * D_MODEL
        # encoder layers (2 layers): self-attention + FFN per layer
        # attention: 3 * (patch_num * d_model * d_model) + patch_num * d_model * d_model
        attn_macs = 4 * patch_num * D_MODEL * D_MODEL
        # FFN: 2 * patch_num * d_model * d_ff
        ffn_macs = 2 * patch_num * D_MODEL * D_FF
        encoder_macs = E_LAYERS * (attn_macs + ffn_macs)
        # head: (d_model * patch_num) * pred_len
        head_macs = D_MODEL * patch_num * PRED_LEN
        macs = patch_proj_macs + encoder_macs + head_macs
    ...
    dummy_dec_cpu = torch.zeros(1, PRED_LEN, ENC_IN)
    with torch.no_grad():
        for _ in range(50):
...
def run_shap(model, device):
    ...
    class PowerWrapper(nn.Module):
        ...
        def forward(self, x):
            # x: [B, 30, 1]
            B = x.shape[0]
            x_dec = torch.zeros(B, PRED_LEN, ENC_IN, device=x.device)
            out = self.model(x, None, x_dec, None)  # [B, 6, 1]
            return out.squeeze(-1)  # [B, 6]
...
def write_result_md(train_m, test_m):
    ...
    lines.append(f"| train_stride | {train_m['stride']} |")
