"""Transformer 模型训练+测试+SHAP 脚本

从 Time-Series-Library 改造迁移，使用 TSL 的 Transformer 模型架构。

输入：
  - 30步功率序列 [B, 30, 1]（单变量，已÷30.1归一化）
输出：
  - 6步功率预测 [B, 6, 1] → squeeze 为 [B, 6]
...
CKPT_DIR = Path("/root/code_compare/TSL/transformer/checkpoints")
RESULT_DIR = Path("/root/code_compare/TSL/transformer/result")
...
class PowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [30] → [30, 1] for Transformer input format
...
# 模型前向辅助
# ---------------------------------------------------------------------------
def build_dec_input(batch_x, batch_y, device):
    """构建 decoder 输入: start token (label_len 步真实值) + zeros (pred_len 步)。

    batch_x: [B, 30, 1] encoder 输入
    batch_y: [B, 6] 真实输出
    返回: [B, label_len + pred_len, dec_in]
    """
    B = batch_x.shape[0]
    # start token: encoder 输入的最后 label_len 步
    start_token = batch_x[:, -LABEL_LEN:, :]  # [B, label_len, 1]
    # zeros: 待预测部分
    zeros = torch.zeros(B, PRED_LEN, DEC_IN, device=device)
    dec_inp = torch.cat([start_token, zeros], dim=1)  # [B, label_len+pred_len, 1]
    return dec_inp


def model_forward(model, x_enc, device):
    """Transformer 前向：x_enc [B, 30, 1] → [B, 6]"""
    B = x_enc.shape[0]
    x_mark_enc = None
    x_dec = build_dec_input(x_enc, None, device)
    x_mark_dec = None
    out = model(x_enc, x_mark_enc, x_dec, x_mark_dec)  # [B, label_len+pred_len, c_out]
    out = out[:, -PRED_LEN:, :]  # [B, 6, 1]
    return out.squeeze(-1)  # [B, 6]
...
def test(model, device):
    ...
    try:
        ...
        dummy_x = torch.randn(1, SEQ_LEN, ENC_IN).to(device)
        class MacWrapper(nn.Module):
            ...
            def forward(self, x_enc):
                B = x_enc.shape[0]
                start_token = x_enc[:, -LABEL_LEN:, :]
                zeros = torch.zeros(B, PRED_LEN, DEC_IN, device=x_enc.device)
                x_dec = torch.cat([start_token, zeros], dim=1)
                out = self.model(x_enc, None, x_dec, None)
                return out[:, -PRED_LEN:, :].squeeze(-1)
        ...
        # Transformer MACs 估算
        # Encoder: e_layers * (self_attn + FFN)
        # self_attn: 4 * seq_len * d_model * d_model
        enc_attn_macs = 4 * SEQ_LEN * D_MODEL * D_MODEL
        enc_ffn_macs = 2 * SEQ_LEN * D_MODEL * D_FF
        encoder_macs = E_LAYERS * (enc_attn_macs + enc_ffn_macs)
        # Decoder: d_layers * (masked_self_attn + cross_attn + FFN)
        dec_len = LABEL_LEN + PRED_LEN
        dec_self_attn = 4 * dec_len * D_MODEL * D_MODEL
        dec_cross_attn = 2 * dec_len * SEQ_LEN * D_MODEL
        dec_ffn = 2 * dec_len * D_MODEL * D_FF
        decoder_macs = D_LAYERS * (dec_self_attn + dec_cross_attn + dec_ffn)
        # embedding + projection
        embed_macs = SEQ_LEN * ENC_IN * D_MODEL + dec_len * DEC_IN * D_MODEL
        proj_macs = dec_len * D_MODEL * C_OUT
        macs = encoder_macs + decoder_macs + embed_macs + proj_macs
    ...
    cpu_model = model.cpu()
    dummy_x_cpu = torch.randn(1, SEQ_LEN, ENC_IN)
    with torch.no_grad():
        for _ in range(50):
...
def run_shap(model, device):
    ...
    bg_size = 200
    print(f"  background: {bg_size} (训练集全量采样)")
    print(f"  explain: {n_explain} (测试集全量采样)")
    ...
    class PowerWrapper(nn.Module):
        ...
        def forward(self, x):
            # x: [B, 30, 1]
            B = x.shape[0]
            start_token = x[:, -LABEL_LEN:, :]
            zeros = torch.zeros(B, PRED_LEN, DEC_IN, device=x.device)
            x_dec = torch.cat([start_token, zeros], dim=1)
            out = self.model(x, None, x_dec, None)  # [B, label_len+pred_len, c_out]
            out = out[:, -PRED_LEN:, :]  # [B, 6, 1]
            return out.squeeze(-1)  # [B, 6]
    ...
    try:
        ...
        try:
            ...
            def kernel_forward(x_np):
                ...
                x_tensor = x_tensor.reshape(-1, SEQ_LEN, ENC_IN)
                with torch.no_grad():
...
def write_result_md(train_m, test_m):
    lines = []
    lines.append("# Transformer 训练与测试结果\n")
    lines.append(
    ...
    lines.append(f"| train_label_len | {train_m['label_len']} |")
    ...
    lines.append(f"| train_d_layers | {train_m['d_layers']} |")
    ...
    lines.append(f"| train_dec_in | {train_m['dec_in']} |")
    lines.append(f"| train_c_out | {train_m['c_out']} |")
    ...
    lines.append(f"- SHAP background samples: 200 (从训练集全量采样)")
    lines.append(f"- SHAP explain samples: 1000 (从测试集全量采样)")
