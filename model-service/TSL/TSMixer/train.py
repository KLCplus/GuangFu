"""TSMixer 模型训练+测试+SHAP 脚本

从 Time-Series-Library 改造迁移，使用 TSL 的 TSMixer 模型架构。
...
CKPT_DIR = Path("/root/code_compare/TSL/TSMixer/checkpoints")
RESULT_DIR = Path("/root/code_compare/TSL/TSMixer/result")
...
class PowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [30] → [30, 1] for TSMixer input format
...
def build_model():
    """构建 TSL TSMixer 模型。"""
    from models.TSMixer import Model as TSMixerModel
...
def test(model, device):
    ...
    try:
        ...
        # TSMixer: e_layers ResBlocks + projection
        macs = 0
        for _ in range(E_LAYERS):
            # temporal: Linear(seq_len, d_model) + Linear(d_model, seq_len) (per variable)
            macs += ENC_IN * (SEQ_LEN * D_MODEL + D_MODEL * SEQ_LEN)
            # channel: Linear(enc_in, d_model) + Linear(d_model, enc_in) (per time step)
            macs += SEQ_LEN * (ENC_IN * D_MODEL + D_MODEL * ENC_IN)
        # projection: Linear(seq_len, pred_len) (per variable)
        macs += ENC_IN * SEQ_LEN * PRED_LEN
