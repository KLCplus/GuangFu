"""DLinear 模型训练+测试+SHAP 脚本

从 Time-Series-Library 改造迁移，使用 TSL 的 DLinear 模型架构。
...
CKPT_DIR = Path("/root/code_compare/TSL/dlinear/checkpoints")
RESULT_DIR = Path("/root/code_compare/TSL/dlinear/result")
...
class PowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [30] → [30, 1] for DLinear input format
...
def test(model, device):
    ...
    try:
        ...
        # DLinear MACs: 2 个 Linear(seq_len -> pred_len) + decomposition
        # 每个 Linear: seq_len * pred_len MACs
        linear_macs = 2 * SEQ_LEN * PRED_LEN
        # decomposition (moving_avg): seq_len MACs
        decomp_macs = SEQ_LEN
        macs = linear_macs + decomp_macs
