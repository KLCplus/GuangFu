...
public class WeatherService {
    ...
    public CurrentWeatherVO current(Long stationId) {
        ...
        WeatherDataDO cached = latestCurrent(stationId);
        ...
        try {
            ...
            WeatherDataDO saved = currentDO(stationId, result);
            upsert(saved);
    ...
    public List<WeatherForecastVO> forecast(Long stationId) {
        PowerStationDO station = requireCoordinates(stationId);
        List<WeatherDataDO> cached = forecastRows(stationId);
        ...
        try {
            ...
            List<WeatherDataDO> saved = results.stream()
                .map(result -> forecastDO(stationId, result)).toList();
